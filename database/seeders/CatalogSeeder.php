...
class CatalogSeeder extends Seeder
{
        ...
        TenantContext::for($company, function () use ($company) {
            ...
            foreach ($sizes as $sizeData) {
                $size = TireSize::firstOrCreate(
                    ['code' => $sizeData['code'], 'company_id' => $company->id],
                    $sizeData + ['uneven_wear_threshold_mm' => 3, 'company_id' => $company->id]
                );
            ...
            foreach (UnitConfigurationCatalog::unitTypes() as $type) {
                UnitType::create($type + ['is_active' => true, 'company_id' => $company->id]);
            }

            foreach (UnitConfigurationCatalog::powered() as $layout) {
                $this->seedLayout($layout, 'POWERED', ['TRACTOR', 'CAMION'], $company->id);
            }
            foreach (UnitConfigurationCatalog::trailers() as $layout) {
                $this->seedLayout($layout, 'TRAILER', $layout['compatible_types'], $company->id);
            }
    ...
    private function seedLayout(array $layout, string $appliesTo, array $compatibleTypes, int $companyId): void
    {
